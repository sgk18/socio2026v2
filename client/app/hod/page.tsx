...
type HodTab = "pending" | "reviewed" | "analytics";
...
function HodDashboard() {
  ...
  const [activeTab, setActiveTab] = useState<HodTab>(
    (searchParams.get("tab") as HodTab) || "pending"
  );
  ...
  const HOD_PAGE_SIZE = 8;

  function switchTab(tab: HodTab) {
    setActiveTab(tab);
    setPage(1);
    setSearchQuery("");
    setExpanded(null);
  ...
  const filtered = q
    ...
    : visible;
  const totalPages = Math.max(1, Math.ceil(filtered.length / HOD_PAGE_SIZE));
  const safePage = Math.min(Math.max(1, page), totalPages);
  const pageItems = filtered.slice((safePage - 1) * HOD_PAGE_SIZE, safePage * HOD_PAGE_SIZE);
        ...
        {!loading && activeTab !== "analytics" && (
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-8">
            {[
              ...
              { label: "Approved", count: reviewedItems.filter((r) => hodStatus(r) === "approved").length,   color: "text-green-700",  bg: "bg-green-50" },
              { label: "Returned", count: reviewedItems.filter((r) => hodStatus(r) === "rejected").length,   color: "text-amber-700",  bg: "bg-amber-50" },
            ].map((s) => (
          ...
          <>
            ...
            <div className="flex items-end justify-between gap-4 border-b border-slate-200 mb-7">
              <div className="flex items-center gap-8">
                {(["pending", "reviewed", "analytics"] as HodTab[]).map((tab) => (
                  <button
                    key={tab}
            ...
            {/* ── Analytics view ── */}
            {activeTab === "analytics" && <HodAnalyticsDashboard />}
