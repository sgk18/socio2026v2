...
const FestsPageContent = () => {
      ...
      .then((data) => {
        if (data && Array.isArray(data.fests)) {
          const normalizedFests: Fest[] = data.fests.map((fest: any) => ({
            ...
            campus_hosted_at: safeText(fest?.campus_hosted_at ?? fest?.campusHostedAt, "") || null,
            allowed_campuses: fest?.allowed_campuses ?? fest?.allowedCampuses ?? null,
            venue: safeText(fest?.venue, "") || null,
            location: safeText(fest?.location, "") || null,
  ...
  useEffect(() => {
    setCurrentPage(1);
  }, [activeFilter, searchQuery, selectedCampus]);
